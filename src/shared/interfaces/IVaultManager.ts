...
export interface IVaultManager {
  ...
  getVaultStats(): Promise<{ totalNotes: number; totalFolders: number; totalTags: number; totalLinks: number }>;
  ...
  listBackups(): Promise<Array<{ timestamp: string; path: string; relPath: string }>>;
