...
export class VaultPool {
  ...
  private initializing = new Map<string, Promise<VaultEntry>>();
  ...
  getVault(vaultPath: string): VaultEntry {
    ...
    if (!entry) throw new Error(`Vault not found: ${vaultPath}`);
    return entry;
  }
  ...
  async addVault(vaultPath: string, acl?: FolderACL, enforceOntology = false): Promise<VaultEntry> {
  ...
  async removeVault(vaultPath: string): Promise<boolean> {
    const resolved = path.resolve(vaultPath);
  ...
  listVaults(): Array<{ path: string; name?: string; tags?: string[] }> {
  ...
  async initializeComponents(entry: VaultEntry, embedProvider?: any, persistence?: any): Promise<void> {
