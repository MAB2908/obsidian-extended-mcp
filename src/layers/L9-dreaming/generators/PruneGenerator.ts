...
export function generatePruneCandidates(
  topics: DreamTopic[],
  opts: PruneGeneratorOptions = {},
): PruneCandidate[] {
  ...
  for (const topic of topics) {
    ...
    if (topic.signals.maturity === 'core') continue;

    const ageMs = now - topic.mtimeMs;
    const staleMs =
      topic.signals.maturity === 'draft' ? staleDaysDraft * DAY_MS : staleDaysValidated * DAY_MS;
