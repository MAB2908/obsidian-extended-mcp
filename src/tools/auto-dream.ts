...
export function createAutoDreamTools(): ToolHandler[] {
  return [
    {
      name: 'auto_dream_run',
      description:
        'L9-Dreaming Auto: Run the automated dreaming pipeline (scan → safe auto-apply) on a vault. ' +
      ...
      handler: async (args) => {
        ...
        const watch = !!a.watch;
        const cronHours = (a.cronHours as number) || 0;
        ...
        if (cronHours > 0) argv.push('--cron', String(cronHours));
    ...
    },
    {
      ...
      description:
        'L9-Dreaming Auto: Install OS-level scheduler for auto-dream (Windows Task Scheduler / macOS launchd / Linux cron).',
      inputSchema: {
        type: 'object',
        properties: {
          vaultPath: {
      ...
      handler: async (args) => {
        const a = args as Record<string, unknown>;
        const vaultPath = a.vaultPath as string;
        const intervalHours = (a.intervalHours as number) || 24;
    ...
    },
    {
      ...
      description:
        'L9-Dreaming Auto: Read the latest auto-dream log for a vault.',
      inputSchema: {
        type: 'object',
        properties: {
          vaultPath: {
