...
export interface SecurityPolicy {
  ...
  approval?: { mode?: 'auto' | 'interactive' | 'strict'; optInTools?: string[] };
}
...
export class SecurityEngine {
  constructor(
    ...
    public readonly acl: FolderACL,
    public readonly gate: OperationGate,
    public readonly audit: AuditLogger,
    public readonly sandbox: Sandbox,
    public readonly approval: ApprovalEngine = new ApprovalEngine(),
  ) {}
  ...
  authorize(toolName: string, args?: Record<string, unknown>): AuthResult {
    ...
    if (!gateResult.allowed) {
      const result: AuthResult = { allowed: false, reason: gateResult.reason };
  ...
  verifyToken(provided?: string): { valid: boolean; reason?: string } {
    ...
    if (!expected) return { valid: true }; // dev mode, no token required
  ...
  private isReadOp(toolName: string): boolean {
    ...
    if (mabsReadOps.includes(toolName)) return true;
  ...
  private isWriteOp(toolName: string, args?: Record<string, unknown>): boolean {
    // Explicitly destructive CLI tools
    const destructiveCli = ['cli_eval', 'cli_plugin', 'cli_command'];
    // Unclassified tools that need write classification
    const unclassifiedWrite = ['build_index'];
    if (unclassifiedWrite.includes(toolName)) return true;
    ...
    const dreamWriteOps = ['dream_finalize', 'dream_undo'];
    if (dreamWriteOps.includes(toolName)) return true;
    ...
    if (mabsWriteOps.includes(toolName)) return true;
