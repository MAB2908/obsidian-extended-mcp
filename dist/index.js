...
import { OllamaEmbeddingProvider, OpenAIEmbeddingProvider } from './layers/L4-semantic/EmbeddingProvider.js';
...
async function main() {
    ...
    async function initializeVaultEntry(entry, embedProviderArg, adapter) {
    ...
    if (llmConfig.anthropicKey) {
        adapter.registerProvider(new AnthropicProvider({
            apiKey: llmConfig.anthropicKey,
            model: llmConfig.anthropicModel,
        }));
    }
    if (llmConfig.ollamaBaseUrl) {
        adapter.registerProvider(new OllamaProvider({
    ...
    if (autoDreamingConfig.enabled) {
        ...
        function runAutoDream(vaultPath) {
            const args = [scriptPath, vaultPath];
            if (autoDreamingConfig.dryRun)
                args.push('--dry-run');
            if (autoDreamingConfig.watch)
                args.push('--watch');
