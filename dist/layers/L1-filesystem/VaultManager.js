...
import { TagEngine } from '../../shared/TagEngine.js';
...
export class VaultManager {
    ...
    async appendNote(relPath, content) {
        if (!this.acl.isWriteAllowed(relPath)) {
            throw new AclDeniedError(relPath, 'write');
        }
        const full = await this.resolve(relPath);
    ...
    async patchNote(relPath, target, operation, replacement) {
        if (!this.acl.isWriteAllowed(relPath)) {
            throw new AclDeniedError(relPath, 'write');
        }
        if (!target) {
    ...
    async listAllTags() {
        ...
        if (this.cacheGeneration === startGen) {
            if (!this.cache)
                this.cache = { stats: { totalNotes: 0, totalFolders: 0, totalTags: 0, totalLinks: 0 }, tags: counts };
            this.cache.tags = counts;
        }
        return counts;
    }
    // Internal helpers
    async readFileSafe(fullPath) {
        try {
                ...
                try {
                    if (!overwrite) {
                        try {
                            ...
                            const code = e && typeof e === 'object' && 'code' in e ? e.code : '';
                            if (code !== 'ENOENT')
                                throw e; // Re-throw permission errors (VM-003)
                        }
                    }
                    let backupPath;
    ...
    async rollback(relPath, timestamp) {
        if (!this.acl.isWriteAllowed(relPath)) {
            throw new AclDeniedError(relPath, 'write');
        }
        const backupsDir = path.join(this.vaultPath, fsConfig.backupDir);
    ...
    async collectMarkdownFiles(relDir) {
        const full = await this.resolve(relDir || '.');
        const results = [];
        const walk = async (dir, prefix) => {
            const entries = await fs.readdir(dir, { withFileTypes: true });
            for (const e of entries) {
                if (e.name.startsWith('.') || e.isSymbolicLink())
                    continue;
    ...
    async manageTags(relPath, action, tags) {
        if (!this.acl.isWriteAllowed(relPath)) {
            throw new AclDeniedError(relPath, 'write');
        }
    ...
    async checkOntology(relPath, content, frontmatter) {
        const parsed = matter(content);
        const effectiveFrontmatter = frontmatter ?? parsed.data;
        const fmTags = Array.isArray(effectiveFrontmatter.tags) ? effectiveFrontmatter.tags.map(String) : [];
    ...
    async batchEdit(filter, operation, target, replacement, preview = false) {
        const files = await this.collectMarkdownFiles(filter.folder ?? '');
        const paths = [];
        for (const f of files) {
            if (filter.glob && !this.globMatch(f, filter.glob))
                continue;
            if (filter.tag) {
                const note = await this.readNote(f, { includeContent: false });
                if (!note.tags.includes(filter.tag))
                    continue;
            }
            paths.push(f);
        }
        ...
        for (const f of paths) {
            if (!this.acl.isWriteAllowed(f))
                continue;
            if (!this.acl.isReadAllowed(f))
                continue;
