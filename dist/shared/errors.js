...
export class PathSecurityError extends FileSystemError {
    constructor(path) {
        super('E101', `Path traversal detected or path outside vault: ${path}`, {
            severity: 'fatal',
...
export class FileLockedError extends FileSystemError {
    constructor(path) {
        super('E102', `Resource busy or locked: ${path}`, {
            severity: 'warning',
...
export class PermissionDeniedError extends FileSystemError {
    constructor(path) {
        super('E103', `Permission denied: ${path}`, {
            severity: 'fatal',
...
export class CorruptedCacheError extends FileSystemError {
    constructor(reason) {
        super('E104', `Cache corrupted: ${reason}`, {
            severity: 'warning',
...
export class FileExistsError extends FileSystemError {
    constructor(path) {
        super('E107', `File already exists: ${path}. Use overwrite=true to replace.`, {
            severity: 'error',
...
export class UnknownOperationError extends FileSystemError {
    constructor(operation) {
        super('E108', `Unknown operation: ${operation}`, {
            severity: 'error',
...
export class NoBackupError extends FileSystemError {
    constructor(path) {
        super('E110', `No backup found for ${path}`, {
            severity: 'error',
...
export class WriteFailedError extends FileSystemError {
    constructor(path, retries) {
        super('E111', `Write failed after ${retries} retries: ${path}`, {
            severity: 'error',
...
export class CliNotFoundError extends CliError {
    constructor() {
        super('E201', 'Obsidian CLI not found. Is the CLI plugin installed?', {
            severity: 'warning',
...
export class CliTimeoutError extends CliError {
    constructor(timeoutMs) {
        super('E202', `CLI command timed out after ${timeoutMs}ms`, {
            severity: 'warning',
...
export class UnknownCliActionError extends CliError {
    constructor(action, context) {
        super('E203', `Unknown ${context} action: ${action}`, {
            severity: 'error',
...
export class CliResponseError extends CliError {
    constructor(details) {
        super('E204', `CLI returned error: ${details}`, {
            severity: 'error',
...
export class CliExitError extends CliError {
    constructor(code, stderr) {
        super('E206', `CLI exited with code ${code}${stderr ? ': ' + stderr : ''}`, {
            severity: 'error',
...
export class CliParseError extends CliError {
    constructor(details) {
        super('E207', `Failed to parse CLI output: ${details}`, {
            severity: 'error',
...
export class RestQueryError extends RestError {
    constructor(query, details) {
        super('E301', `Dataview query failed: ${query}${details ? ' — ' + details : ''}`, {
            severity: 'error',
...
export class RestNotFoundError extends RestError {
    constructor(path) {
        super('E303', `REST endpoint or note not found: ${path}`, {
            severity: 'error',
...
export class RestAuthError extends RestError {
    constructor(details) {
        super('E304', `REST API authentication failed${details ? ': ' + details : ''}`, {
            severity: 'fatal',
...
export class RestTimeoutError extends RestError {
    constructor(timeoutMs) {
        super('E305', `REST API request timed out after ${timeoutMs}ms`, {
            severity: 'warning',
            recovery: 'Increase timeout or check Obsidian responsiveness.',
            retryable: true,
            maxRetries: 1,
...
export class ConfigInvalidError extends ConfigError {
    constructor(reason) {
        super('E901', `Config invalid: ${reason}`, {
            severity: 'fatal',
...
export class VaultPathNotFoundError extends ConfigError {
    constructor(path) {
        super('E902', `Vault path not found: ${path}`, {
            severity: 'fatal',
...
export class AuditLogWriteFailedError extends ConfigError {
    constructor(details) {
        super('E903', `Audit log write failed${details ? ': ' + details : ''}`, {
            severity: 'error',
...
export class MemoryLimitExceededError extends ConfigError {
    constructor(limit) {
        super('E904', `Memory limit exceeded: ${limit}`, {
            severity: 'fatal',
...
export class AclDeniedError extends SecurityError {
    constructor(path, action) {
        super('E401', `${action.charAt(0).toUpperCase() + action.slice(1)} denied by ACL: ${path}`, {
            severity: 'fatal',
...
export class OntologyViolationError extends SecurityError {
    constructor(path, violations) {
        super('E402', `Ontology violation in ${path}: ${violations.join('; ')}`, {
            severity: 'error',
...
export class LLMHttpError extends LLMProviderError {
    constructor(status, body) {
        super('E801', `LLM provider HTTP error ${status}: ${body}`, {
            severity: 'error',
