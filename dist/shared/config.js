...
function yamlVal(section, key) {
    if (!yamlConfig)
        return undefined;
    const sec = yamlConfig[section];
    if (!sec)
        return undefined;
    return sec[key];
}
...
export const serverConfig = {
    ...
    httpCorsOrigins: envList('MCP_HTTP_CORS_ORIGINS', yamlVal('server', 'httpCorsOrigins') ?? []),
...
export const securityConfig = {
    ...
    approvalMode: env('APPROVAL_MODE', yamlVal('security', 'approvalMode') ?? 'auto'),
    ...
    safeZones: envList('SAFE_ZONES', yamlVal('security', 'safeZones') ?? ['raw/', 'sessions/']),
    /** Writable paths */
    writePaths: envList('WRITE_PATHS', yamlVal('security', 'writePaths') ?? ['*']),
    /** Forbidden paths */
    forbiddenPaths: envList('FORBIDDEN_PATHS', yamlVal('security', 'forbiddenPaths') ?? ['.git/', '.obsidian/', '.trash/']),
    ...
    sandboxAllowedGlobals: envList('SANDBOX_ALLOWED_GLOBALS', yamlVal('security', 'sandboxAllowedGlobals') ?? ['app', 'DataviewAPI', 'moment', 'MetadataCache']),
    /** Audit log format */
...
export const fileTypeConfig = {
    ...
    textExtensions: envList('FILE_TEXT_EXTENSIONS', yamlVal('fileType', 'textExtensions') ?? [
