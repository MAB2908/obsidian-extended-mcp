...
export async function validatePath(vaultRoot, requestedPath) {
...
export function safeJsonParse(raw, maxSize = 10 * 1024 * 1024, maxDepth = 50) {
    if (raw.length > maxSize) {
        throw new Error(`JSON payload exceeds maximum size of ${maxSize} bytes`);
    }
    const obj = JSON.parse(raw);
    const checkDepth = (value, depth) => {
        if (depth > maxDepth) {
